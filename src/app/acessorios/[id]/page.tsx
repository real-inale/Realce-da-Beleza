...
export default function AccessoryDetail({ params }: Props) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <h2 className="text-2xl font-title mb-4">Acessório não encontrado</h2>
          <Link href="/acessorios" className="text-gold font-body hover:underline">Voltar à listagem</Link>
        </div>
      </div>
    )
  }
  ...
  const whatsappUrl = `https://wa.me/244943700307?text=${encodeURIComponent(`Olá, gostaria de reservar o acessório ${accessory.title}. Está disponível?`)}`
        ...
        <div className="flex flex-col">
          ...
          <div className="mb-6 border-y border-gray-100 py-3 flex items-center gap-3">
             <span className={`inline-block px-3 py-1 text-sm font-semibold rounded ${accessory.availability === 'available' ? 'bg-green-100 text-green-800' : accessory.availability === 'low' ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-800'}`}>
                 {accessory.availability === 'available' ? 'Disponível' : accessory.availability === 'low' ? 'Poucas unidades' : 'Indisponível'}
             </span>
